...
export const uploadFile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error while saving file metadata." });
  }
};
...
export const deleteFile = async (req: Request, res: Response) => {
  try {
    ...
    const deletedAnnotations = await Annotation.deleteMany({
      documentId: req.params.fileId,
    });
    console.log(
      `Deleted ${deletedAnnotations.deletedCount} annotations for file ${req.params.fileId}`
    );
    ...
    if (error.message && error.message.includes("File not found")) {
      ...
      return res
        ...
        .json({
          message:
            "File metadata and annotations deleted (actual file was already missing).",
        });
    }
